    ...
    function BarChart({items,max,aylik=false}:{items:{label:string;total:number;values:Record<string,number>}[];max:number;aylik?:boolean}){return <div className="mt-5 overflow-x-auto"><div className={aylik?"min-w-[900px]":"min-w-[760px]"}><div className={aylik?"relative h-[320px]":"relative h-[350px]"}><div className="pointer-events-none absolute inset-x-0 bottom-10 top-5 flex flex-col justify-between">{[1,2,3,4,5].map(i=><div key={i} className="border-t border-dashed border-slate-800"/>)}</div><div className="absolute inset-0 flex items-end gap-3 px-2">{items.map(it=><div key={it.label} className="flex min-w-[62px] flex-1 flex-col items-center justify-end"><div className="mb-2 rounded-full bg-[#07111f] px-2 py-1 text-[9px] font-black">{it.total}</div><div className={`flex items-end gap-[2px] ${aylik?"h-[225px]":"h-[255px]"}`}>{NEDENLER.map(n=>{const v=it.values[n.ad]||0,h=v?Math.max(6,(v/max)*(aylik?200:225)):0;return <div key={n.ad} title={`${it.label} - ${n.ad}: ${v}`} className={aylik?"w-[5px] rounded-t":"w-[7px] rounded-t-md"} style={{height:`${h}px`,backgroundColor:n.renk}}/>})}</div><div className="mt-3 w-full border-t border-slate-700 pt-2 text-center text-[10px] font-black text-slate-500">{it.label}</div></div>)}</div></div></div></div>}
