...
const EVENT_KAPAT="trafo-harita-kapat";
...
export default function HaritaEntegrasyon(){
  ...
  useEffect(()=>{
    ...
    window.addEventListener(EVENT_KAPAT,kapat);
    return()=>{window.removeEventListener(EVENT_AC,ac);window.removeEventListener(EVENT_KAPAT,kapat);};
  },[]);

  useEffect(()=>{
    const ekle=()=>{
      const navlar=Array.from(document.querySelectorAll("nav"));
      navlar.forEach(nav=>{
        if(nav.querySelector('[data-trafo-harita-menu="1"]'))return;
        const butonlar=Array.from(nav.querySelectorAll("button"));
        const hedef=butonlar.find(b=>b.textContent?.includes("Trafo Kayıtları"));
        if(!hedef)return;
        ...
        b.onclick=()=>{
          ...
          const mobilAside=b.closest("aside.lg\\:hidden");
          if(mobilAside){
            const kapatButonu=Array.from(mobilAside.querySelectorAll("button")).find(x=>x.textContent?.trim()==="×") as HTMLButtonElement|undefined;
            kapatButonu?.click();
          }
