...
export default function HaritaEntegrasyon(){
 ...
 return <div className="fixed inset-x-0 bottom-0 top-[76px] z-[29] overflow-y-auto bg-[#f4f7fb] lg:left-[248px]">
  <style jsx global>{`.leaflet-overlay-pane canvas{filter:drop-shadow(0 0 2px #fff) drop-shadow(0 0 1px #fff) saturate(2) contrast(1.35)!important}.leaflet-popup-content-wrapper{box-shadow:0 8px 28px rgba(15,23,42,.28)!important}.leaflet-popup-tip{box-shadow:3px 3px 8px rgba(15,23,42,.15)!important}`}</style>
  ...
  {gecmis!==null&&<div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-[2px]" onMouseDown={e=>{if(e.target===e.currentTarget)setGecmis(null);}}><div className="max-h-[90vh] w-full max-w-5xl overflow-hidden rounded-2xl bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-slate-200 px-5 py-4"><div><div className="text-[10px] font-black uppercase tracking-wider text-violet-600">Trafo Değişim Geçmişi</div><div className="mt-1 text-lg font-black text-slate-900">{ilk?.tr||`Lokasyon ${gecmisLok}`}</div><div className="mt-1 text-xs font-semibold text-slate-500">Lokasyon ID: {gecmisLok} • {gecmisYukleniyor?"Yükleniyor...":`${gecmis.length} değişim kaydı`}</div></div><button onClick={()=>setGecmis(null)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-xl font-bold text-slate-500 hover:bg-slate-200">×</button></div><div className="max-h-[74vh] overflow-y-auto p-4 sm:p-5">{gecmisHata&&<div className="rounded-xl bg-red-50 p-4 text-sm font-bold text-red-700">{gecmisHata}</div>}{gecmisYukleniyor&&<div className="py-10 text-center text-sm font-bold text-slate-400">Geçmiş kayıtları yükleniyor...</div>}{!gecmisYukleniyor&&!gecmisHata&&gecmis.length===0&&<div className="py-10 text-center text-sm font-bold text-slate-400">Bu lokasyon için değişim kaydı bulunamadı.</div>}{!gecmisYukleniyor&&gecmis.map((k:any,i:number)=><div key={k.id??i} className="mb-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm last:mb-0"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50 px-4 py-3"><div className="flex items-center gap-2"><div className="text-sm font-black text-slate-900">{i+1}. Değişim</div><span className="rounded-full px-2.5 py-1 text-[10px] font-black text-white" style={{backgroundColor:nedenRengi(k.degisim_nedeni)}}>{k.degisim_nedeni||"NEDEN BELİRTİLMEMİŞ"}</span></div><div className="rounded-lg bg-violet-100 px-2.5 py-1 text-xs font-black text-violet-700">{tarih(k.tarih)}</div></div><div className="p-4"><div className="grid gap-3 text-xs sm:grid-cols-2 lg:grid-cols-4"><Bilgi ad="İlçe" deger={k.ilce}/><Bilgi ad="Mahalle" deger={k.mahalle}/><Bilgi ad="Trafo" deger={k.tr}/><Bilgi ad="Trafo ID" deger={k.trafo_id}/><Bilgi ad="Lokasyon ID" deger={k.lokasyon_id}/><Bilgi ad="Montaj Tipi" deger={k.trafo_tipi}/><Bilgi ad="Yıl" deger={k.yil}/><Bilgi ad="Ay" deger={k.ay}/></div><div className="mt-4 grid gap-3 lg:grid-cols-2"><TrafoKutusu baslik="Sökülen Trafo" guc={k.sokulen_gucu} gerilim={k.sokulen_gerilim} marka={k.sokulen_markasi} seri={k.sokulen_seri_no} imal={k.sokulen_imal_yili} tip={k.sokulen_trafo_tipi} tamirYili={k.sokulen_tamir_yili} tamirFirma={k.sokulen_tamir_firmasi}/><TrafoKutusu baslik="Takılan Trafo" guc={k.takilan_gucu} gerilim={k.takilan_gerilim} marka={k.takilan_markasi} seri={k.takilan_seri_no} imal={k.takilan_imal_yili} tip={k.takilan_trafo_tipi} tamirYili={k.takilan_tamir_yili} tamirFirma={k.takilan_tamir_firmasi}/></div>{(k.sokulen_yuklenici||k.aciklama)&&<div className="mt-4 grid gap-3 sm:grid-cols-2">{k.sokulen_yuklenici&&<Bilgi ad="Yüklenici" deger={k.sokulen_yuklenici}/>} {k.aciklama&&<Bilgi ad="Açıklama" deger={k.aciklama}/>}</div>}</div></div>)}</div></div></div>}
 </div>;
}
