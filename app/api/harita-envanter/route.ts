...
const PATH="harita/trafo-envanter.zip";
...
export async function GET(){
  try{
    ...
    if(error||!data){
      ...
      if(msg.includes("not found")||msg.includes("object not found")||msg.includes("404"))return NextResponse.json({error:"Merkezi harita envanteri henüz oluşturulmadı."},{status:404});
      throw error||new Error("Harita envanteri indirilemedi.");
    }
    return new NextResponse(data,{status:200,headers:{"Content-Type":"application/zip","Content-Disposition":"inline; filename=trafo-envanteri.zip","Cache-Control":"no-store, max-age=0"}});
  }catch(e:any){
    return NextResponse.json({error:e?.message||"Harita envanteri alınamadı."},{status:500});
  }
}
...
export async function POST(req:NextRequest){
  try{
    ...
    return NextResponse.json({error:e?.message||"Harita envanteri kaydedilemedi."},{status:500});
  }
}
