...
const DRIVE_ROOT_FOLDER_ID = process.env.GOOGLE_DRIVE_TUTANAKLAR_FOLDER_ID || "18zcKMS0BRn18R1tjyhfmuxEdMJmqI1qW";
...
async function getGoogleAccessToken() {
  ...
  if (!email || !rawKey) throw new Error("Google servis hesabı Vercel ortam değişkenleri eksik.");
...
function monthFromFolderName(name: string) {
  const upper = normalizeTr(name);
  const months = ["OCAK","ŞUBAT","MART","NİSAN","MAYIS","HAZİRAN","TEMMUZ","AĞUSTOS","EYLÜL","EKİM","KASIM","ARALIK"];
  return months.find((m) => upper.includes(m)) || "";
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const rootItems = await listFolder(accessToken, DRIVE_ROOT_FOLDER_ID);
    const yearFolder = rootItems.find((x) => x.mimeType === "application/vnd.google-apps.folder" && x.name.trim() === String(yil));
    if (!yearFolder) return NextResponse.json({ error: `${yil} klasörü TUTANAKLAR içinde bulunamadı.` }, { status: 404 });

    const yearItems = await listFolder(accessToken, yearFolder.id);
    const monthFolder = yearItems.find((x) => x.mimeType === "application/vnd.google-apps.folder" && monthFromFolderName(x.name) === ay);
    if (!monthFolder) return NextResponse.json({ error: `${yil} ${ay} klasörü bulunamadı.` }, { status: 404 });

    const monthItems = await listFolder(accessToken, monthFolder.id);
