...
export default function HaritaGelismisIslemler(){
 useEffect(()=>{
  const ekle=()=>{
   ...
   const k=kartlar.find(x=>x.querySelector&&x.children.length<8);if(k&&!k.dataset.uyumsuzKlik){k.dataset.uyumsuzKlik="1";k.style.cursor="pointer";k.title="Uyumsuz trafoları haritada göster";k.onclick=()=>{const inp=Array.from(document.querySelectorAll<HTMLInputElement>('input[placeholder*="Trafo"]'))[0];if(inp){const setter=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value")?.set;setter?.call(inp,"uyumsuz");inp.dispatchEvent(new Event("input",{bubbles:true}));}};}
  };
